...
namespace sylar
{
	...
	Fiber::Fiber(std::function<void()> cb, size_t stacksize, bool run_in_scheduler) : m_cb(cb), m_runInScheduler(run_in_scheduler)
	{
		...
		m_ctx.uc_link = nullptr; // 未设置后继，在运行完MainFunc后协程退出，调用yield返回主协程
	...
	void Fiber::MainFunc()
	{
		std::shared_ptr<Fiber> curr = GetThis(); // GetThis()的shared_from_this()方法让引用计数+1
		assert(curr != nullptr);
		...
		curr.reset(); // 重置cb回调函数，希望它指向nullptr，方便其他线程再次调用这个协程对象
		raw_ptr->yield();
	}
